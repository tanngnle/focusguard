...
export const PLATFORM_TEMPLATES = {
  youtube: {
    ...
    elements: {
      ...
      // Shorts shelf on homepage and watch pages. Union of legacy
      // renderers and current markup: the rich-section-scoped variants
      // cover the post-2023 shelf, which no longer hangs off
      // ytd-reel-shelf-renderer. CSS no-ops on missing selectors, so
      // keeping stale members is harmless.
      shorts: "ytd-rich-section-renderer[is-shorts], ytd-rich-section-renderer:has(a[href*='/shorts/']), ytd-reel-shelf-renderer, #shorts-container, ytd-shorts-grid-renderer",
      // Comments section
      comments: "ytd-comments, #comments",
      // Trending tab content
      trending: "ytd-browse[page-subtype='trending'] ytd-rich-grid-renderer",
      // End screen / related videos on watch pages. YouTube is replacing
      // ytd-compact-*-renderer with lockup view-models, scoped here to the
      // watch page so search/browse lockups are left alone.
      endScreen: "ytd-compact-video-renderer, ytd-compact-movie-renderer, ytd-compact-radio-renderer, ytd-watch-flexy yt-lockup-view-model, ytd-watch-flexy ytd-lockup-view-model, ytd-watch-flexy ytd-lockup-renderer",
    },
  },
 ...
 *
 * Profiles are PARTIAL by design: the popup persists only toggled
 * keys (`toggleElement`), so a stored profile like `{ comments: false }`
 * means "everything else is still on". Any element not explicitly set
 * to `false` is treated as enabled — the exact same semantics the popup
 * uses to render its toggles (`profile[elementName] !== false`).
 *
 * @param {string} domain - The hostname (e.g., "youtube.com", "www.facebook.com")
 * @param {object|null} profile - User's stripping profile (element name → boolean), possibly partial
...
export function getStrippingRules(domain, profile) {
  ...
  // Collect selectors for enabled elements. Undefined keys mean
  // "never toggled" → enabled (default-on), matching the popup UI.
  const rules = [];
  for (const [elementName, selector] of Object.entries(template.elements)) {
    if (profile[elementName] !== false) {
      rules.push(selector);
    }
  }
