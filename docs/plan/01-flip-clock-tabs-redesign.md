# FocusGuard — Flip Clock Redesign, Timer Fix & Popup Tabs
...
7. **Wire into blocked page**: inside `.timer-display` (`blocked.html:53-56`) add the flip-clock markup; **keep `#timer-digits` in DOM as `aria-hidden` source of truth** — it backs `window.__focusguardTimer.getDisplay()` (`blocked.js:488-497`, consumed by `blocked-chat.js:137`) and the a11y path via `#timer-sr-status`. Extend `renderTime()` (`blocked.js:304-308`) to call `flipClock.setTime(formatTime(secs))` while keeping `timerDigits.textContent` writes exactly as today.
